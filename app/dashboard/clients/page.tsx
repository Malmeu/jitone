    ...
    return (
        ...
        >
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
                ...
                <motion.div variants={itemVariants}>
                    <Button
                        ...
                        className="h-14 px-8 rounded-2xl bg-neutral-900 dark:bg-white text-white dark:text-black hover:bg-neutral-800 dark:hover:bg-neutral-100 shadow-xl transition-all active:scale-[0.98] font-bold font-inter"
                    >
            ...
            <motion.div variants={itemVariants} className="grid grid-cols-1 lg:grid-cols-12 gap-4 mb-8">
                ...
                <div className="lg:col-span-4 flex items-center justify-between bg-card rounded-2xl border border-neutral-100 dark:border-neutral-800 shadow-sm px-8 py-4">
                    <div className="flex flex-col">
                        <span className="text-2xl font-black text-foreground leading-none">{clients.length}</span>
                        ...
                        <tbody className="divide-y divide-neutral-50 dark:divide-neutral-800 font-inter">
                            {filteredClients.length === 0 ? (
                                ...
                                filteredClients.map((client) => (
                                    <tr key={client.id} className="group hover:bg-[#FBFBFD]/50 dark:hover:bg-neutral-900/50 transition-all border-l-4 border-l-transparent hover:border-l-primary/30">
                                        ...
                                        <td className="px-8 py-6">
                                            <div className="flex flex-col gap-1.5">
                                                <div className="flex items-center gap-2 text-neutral-600 dark:text-neutral-300 font-bold">
                                        ...
                                        <td className="px-8 py-6">
                                            <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-blue-50 dark:bg-blue-900/20 rounded-xl">
                                                <History size={14} className="text-blue-500" />
                                                <span className="text-xs font-black text-blue-600 dark:text-blue-400">{client.repairs?.[0]?.count || 0}</span>
                                                <span className="text-[10px] font-bold text-blue-500 dark:text-blue-400 uppercase tracking-tight">Réparations</span>
                                            </div>
                                        </td>
                                        <td className="px-8 py-6">
                                        ...
                                        <td className="px-8 py-6 text-right">
                                            <div className="flex items-center justify-end gap-2 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                                                <button onClick={() => handleEdit(client)} className="p-2.5 bg-card text-blue-500 rounded-xl border border-neutral-100 dark:border-neutral-800 hover:bg-blue-50 dark:hover:bg-blue-900/20 shadow-sm transition-all" title="Modifier"><Edit3 size={18} /></button>
                                                <button onClick={() => handleDelete(client.id, client.name)} className="p-2.5 bg-card text-red-500 rounded-xl border border-neutral-100 dark:border-neutral-800 hover:bg-red-50 dark:hover:bg-red-900/20 shadow-sm transition-all" title="Supprimer"><Trash2 size={18} /></button>
                                            </div>
                                        </td>
                                    </tr>
                                ))
                ...
                {showModal && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                        ...
                        <motion.div
                            ...
                            className="relative bg-card rounded-[3rem] shadow-heavy max-w-md w-full overflow-hidden flex flex-col font-inter border border-neutral-100 dark:border-neutral-800"
                        >
                            <div className="p-8 border-b border-neutral-100 dark:border-neutral-800 flex justify-between items-center bg-neutral-50/50 dark:bg-neutral-900/50">
                                <div>
                                    <h2 className="text-xl font-bold text-foreground">{editingClient ? 'Modifier le client' : 'Créer un nouveau client'}</h2>
                                    <p className="text-xs text-neutral-400 font-medium">Prenez soin de remplir les contacts correctly.</p>
                                </div>
                                <button onClick={() => setShowModal(false)} className="w-10 h-10 flex items-center justify-center bg-card hover:bg-neutral-100 dark:hover:bg-neutral-800 rounded-xl border border-neutral-100 dark:border-neutral-800 transition-all active:scale-90"><X className="w-5 h-5 text-neutral-400" /></button>
                            </div>
                            ...
                            <form onSubmit={handleSubmit} className="p-8 space-y-6">
                                <div className="space-y-2">
                                    ...
                                    <div className="relative">
                                        ...
                                        <input
                                            ...
                                            className="w-full pl-11 pr-4 py-4 rounded-2xl border border-neutral-100 dark:border-neutral-800 focus:outline-none focus:ring-4 focus:ring-primary/5 focus:border-primary transition-all bg-neutral-50/50 dark:bg-neutral-900/50 font-bold text-foreground placeholder-neutral-400"
                                ...
                                <div className="pt-4 flex gap-3">
                                    <Button type="button" variant="ghost" onClick={() => setShowModal(false)} className="flex-1 rounded-2xl h-14 font-extrabold text-neutral-400 hover:text-neutral-500 font-inter">Annuler</Button>
                                    <Button type="submit" disabled={submitting} className="flex-[2] rounded-2xl h-14 bg-neutral-900 dark:bg-white text-white dark:text-black font-black hover:bg-neutral-800 dark:hover:bg-neutral-100 shadow-xl transition-all font-inter">
                                        {submitting ? <Loader2 className="w-6 h-6 animate-spin mx-auto text-white dark:text-black" /> : (editingClient ? 'Enregistrer' : 'Créer sa fiche')}
                                    </Button>
                                </div>
                            </form>
                        </motion.div>

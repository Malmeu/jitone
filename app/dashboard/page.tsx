...
export default function DashboardPage() {
    ...
    return (
        <motion.div
            key={isSimplified ? 'simplified' : 'normal'}
            initial="hidden"
            animate="visible"
            ...
            {isSimplified ? (
                <div className="space-y-12">
                    ...
                    <div className="flex items-center justify-between">
                        <div>
                            <span className="px-3 py-1 bg-emerald-500/10 text-emerald-500 text-[10px] font-black uppercase tracking-widest rounded-full border border-emerald-500/20 mb-3 inline-block">Focus Mode Activé</span>
                            <h1 className="text-4xl md:text-5xl font-black text-foreground tracking-tight">Vues d&apos;actions</h1>
                        </div>
                        <div className="text-right hidden md:block">
                            <p className="text-sm font-bold text-neutral-400 uppercase tracking-widest leading-none mb-1">Session</p>
                            <p className="text-2xl font-black text-foreground">{stats.todayRevenue.toLocaleString('fr-DZ')} DA</p>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
                        <motion.button
                            ...
                            className="bg-neutral-900 dark:bg-white text-white dark:text-black p-10 rounded-[3rem] shadow-heavy flex flex-col items-center justify-center text-center gap-6 group relative overflow-hidden h-72"
                        >
                            <div className="absolute inset-0 bg-blue-500/10 opacity-0 group-hover:opacity-100 transition-opacity" />
                            <div className="w-20 h-20 bg-white/10 dark:bg-black/5 rounded-3xl flex items-center justify-center mb-2">
                                <Plus size={40} className="text-blue-400" />
                            </div>
                            <div>
                                <h2 className="text-3xl font-black tracking-tight mb-2">Nouvelle Réparation</h2>
                                <p className="text-neutral-400 dark:text-neutral-500 font-medium">Enregistrez un nouvel appareil</p>
                            </div>
                        </motion.button>
                        ...
                        <motion.button
                            ...
                            className="bg-card border-2 border-dashed border-neutral-100 dark:border-neutral-800 p-10 rounded-[3rem] flex flex-col items-center justify-center text-center gap-6 group h-72"
                        >
                            <div className="w-20 h-20 bg-emerald-500/10 rounded-3xl flex items-center justify-center mb-2">
                                <DollarSign size={40} className="text-emerald-500" />
                            </div>
                            <div>
                                <h2 className="text-3xl font-black text-foreground tracking-tight mb-2">Encaisser Vente</h2>
                                <p className="text-neutral-500 font-medium">Vendre un accessoire ou pièce</p>
                            </div>
                        </motion.button>
                        ...
                        <motion.button
                            ...
                            className="bg-card border-2 border-neutral-100 dark:border-neutral-800 p-10 rounded-[3rem] shadow-soft flex flex-col items-center justify-center text-center gap-6 group h-72"
                        >
                            <div className="w-20 h-20 bg-amber-500/10 rounded-3xl flex items-center justify-center mb-2 text-amber-500">
                                <Activity size={32} />
                            </div>
                            <div>
                                <h2 className="text-3xl font-black text-foreground tracking-tight mb-2">État du Stock</h2>
                                <p className="text-neutral-500 font-medium">Vérifier les pièces disponibles</p>
                            </div>
                        </motion.button>

                        <div className="bg-card p-10 rounded-[3rem] shadow-soft border border-neutral-100 dark:border-neutral-800 h-72 flex flex-col justify-between">
                            <div className="flex items-center gap-3">
                                <Activity size={20} className="text-primary" />
                                <h3 className="font-bold text-xl tracking-tight">En cours</h3>
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <p className="text-3xl font-black text-foreground">{stats.todayRepairs}</p>
                                    <p className="text-[10px] font-black uppercase text-neutral-400 tracking-widest">Aujourd&apos;hui</p>
                                </div>
                                <div>
                                    <p className="text-3xl font-black text-emerald-500">{stats.readyRepairs}</p>
                                    <p className="text-[10px] font-black uppercase text-neutral-400 tracking-widest">Prêts</p>
                                </div>
                            </div>
                            <Button variant="ghost" className="w-full rounded-2xl h-12 font-bold" onClick={() => router.push('/dashboard/repairs')}>

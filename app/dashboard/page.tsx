    ...
    return (
        ...
        >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
                <motion.div
                    variants={itemVariants}
                    className="bg-white p-8 rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.03)] border border-neutral-100 hover:shadow-[0_20px_40px_rgb(0,0,0,0.06)] transition-all group"
                >
                    ...
                    <div className="flex items-baseline gap-2">
                        <h3 className="text-4xl font-black text-neutral-900">{stats.todayRepairs}</h3>
                ...
                >
                    ...
                    <div className="flex items-baseline gap-2">
                        <h3 className="text-4xl font-black text-neutral-900">{stats.readyRepairs}</h3>
                ...
                <motion.div
                    variants={itemVariants}
                    className="bg-neutral-900 p-8 rounded-[2.5rem] shadow-[0_20px_40px_rgba(0,0,0,0.1)] text-white relative overflow-hidden group col-span-1 md:col-span-2 lg:col-span-1"
                >
                    <div className="absolute top-0 right-0 w-32 h-32 bg-primary/20 rounded-full blur-[80px] -mr-16 -mt-16" />

                    <div className="flex justify-between items-center mb-6">
                        <div className="w-14 h-14 bg-white/10 backdrop-blur-md text-primary rounded-2xl flex items-center justify-center">
                            <Wallet size={28} />
                        </div>
                        <div className="flex gap-1.5 p-1 bg-white/5 rounded-xl border border-white/5">
                            {(['today', 'month'] as const).map((f) => (
                                <button
                                    ...
                                    className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${revenueFilter === f ? 'bg-white text-black' : 'text-neutral-400 hover:text-white'}`}
                                >
                                    {f === 'today' ? 'Jour' : 'Mois'}
                                </button>
                            ))}
                    ...
                    <button
                        ...
                        className="mt-8 py-3 w-full bg-white/5 hover:bg-white/10 rounded-xl text-sm font-bold transition-all border border-white/5"
                    >
            ...
            <motion.div
                variants={itemVariants}
                className="bg-white rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.02)] border border-neutral-100 overflow-hidden"
            >
                <div className="p-8 border-b border-neutral-50 flex justify-between items-center">
                    <div className="flex items-center gap-3">
                        ...
                        <h3 className="font-bold text-xl text-neutral-900 tracking-tight">Activité Récente</h3>
                    </div>
                ...
                {recentRepairs.length === 0 ? (
                    <div className="p-20 text-center">
                        <Wrench className="w-16 h-16 text-neutral-100 mx-auto mb-6" />
                        ...
                        <table className="w-full text-sm text-left">
                            ...
                            <tbody className="divide-y divide-neutral-50">
                                {recentRepairs.map((repair) => (
                                    <tr
                                        key={repair.id}
                                        className="group hover:bg-[#FBFBFD] transition-all cursor-pointer"
                                        onClick={() => router.push('/dashboard/repairs')}
                                    >
                                        ...
                                        <td className="px-8 py-6">
                                            <div className="font-bold text-neutral-900 mb-0.5">{repair.client?.name || 'Inconnu'}</div>
                                            <div className="text-neutral-400 text-xs font-medium">{repair.item}</div>
                                        </td>
                                        <td className="px-8 py-6">
                                            <span className={`${statusColors[repair.status]} px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-tight`}>
